...
# -----------------------------------------------------------------------
# create a fit table with variables values to use during simulation
tbl_name = 'fit'
col_name = [ 'variable_id', 'fit_value' ]
...
for variable_id in range( len(variable_dict) ) :
	...
	if variable_type in [ 'mulstd_value', 'mulstd_dage', 'mulstd_dtime' ] :
		...
		fit_value = 5e-3 * (rate_id + 1)
	else :
		assert False
	fit_row = [ variable_id, fit_value ]
	row_list.append( fit_row )
...
for simulate_id in range( len(simulate_dict) ) :
	...
	assert meas_value != fit_value
	assert abs( meas_value - fit_value ) < 2.0 * meas_std
