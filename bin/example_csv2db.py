...
# option_csv, data_csv
if len(sys.argv) != 2 :
	msg = 'usage: example_csv2db.py option_csv data_csv'
	sys.exit(msg)
option_csv = sys.arg[1]
data_csv   = sys.arg[2]
for file_name in option_csv data_csv :
	if not file_name.endswith('.csv') :
		msg  = 'example_csv2db.py: file name ' + file_name
		msg += ' does not end with .csv'
		sys.exit(msg)
	if not os.path.isfile(file_name) :
		msg = 'example_csv2db.py: cannot find file ' + file_name
		sys.exit(msg)
# -----------------------------------------------------------------------------
# file_option
...
for name in required :
	if not name in file_option :
		msg = 'example_csv2db: ' + name + ' not in option_csv ' + option_csv
		sys.exit(msg)
if 'omega' not in file_option['non_zero_rates'] :
	msg = 'example_csv2db: omega not in non_zero_rates in ' + option_csv
	sys.exit(msg)
# -----------------------------------------------------------------------------
# file_data
...
for row in file_data :
	...
	if row['integrand'] == 'mtother' :
		...
		if row['age_lower'] != row['age_upper'] :
			msg  = 'example_csv2db.py: line ' + str(line)
...
if not mtother_found
	msg = 'example_csv2db.py: no mtother data in ' + data_csv
	sys.exit(msg)
# -----------------------------------------------------------------------------
...
rate_list    = file_option['non_zero_rates'].split() :
