...
hash_key='30d96b497d059384396b6fc0ef5b30210730aeeb'
version='2026.7.2'
# --------------------------------------------------------------------------
name='tools/get_cppad_mixed.sh'
if [ $0 != $name ]
then
...
# dismod_at_prefix,
# cmake_libdir
eval $(tools/install_settings.py)
# ---------------------------------------------------------------------------
...
then
    tools/build_type.sh get_cppad_mixed.sh $dismod_at_prefix $build_type
fi
# ---------------------------------------------------------------------------
# cd into external
if [ ! -e external ]
...
cppad_mixed_dir=$(pwd)
echo "edit $cppad_mixed_dir/tools/run_cmake.sh"
sed \
    ...
    tools/run_cmake.sh > run_cmake.$$
mv run_cmake.$$ tools/run_cmake.sh
chmod +x tools/run_cmake.sh
#
# check edit
list="
    ^verbose_makefile='no'
...
for pattern in $list
do
    if ! grep "$pattern" tools/run_cmake.sh > /dev/null
    then
        echo "get_cpapd_mixed.sh: pattern=$pattern"
        echo "Edit of $cppad_mixed_dir/tools/run_cmake.sh failed"
        exit 1
    fi
done
# $extra_cxx_flags my have spaces in it
if ! grep "^extra_cxx_flags='$extra_cxx_flags'"  tools/run_cmake.sh > /dev/null
then
...
run_test='false'
replace='true'
if echo_eval tools/example_install.sh $run_test $replace
then
    ok='yes'
fi
