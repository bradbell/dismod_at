...
if [ ! -e 'tools/speed.sh' ]
then
    echo 'tools/speed.sh must be run from its parent directory'
    exit 1
fi
if [ "$1" == '' ]
then
    echo 'usage: tools/speed.sh branch1'
    echo 'example: tools/speed.sh master'
    echo 'see settings at top of tools/speed.sh'
    exit 1
fi
branch1="$1"
# -----------------------------------------------------------------------------
...
then
    echo "tools/speed.sh: branch2 cannot be original_branch = $original_branch"
    exit 1
fi
if [ "$branch1" == "$branch2" ]
then
    echo 'tools/speed.sh: branch1 and branch2 must be different'
...
then
    echo 'tools/speed.sh: install_settings.py build_type is not release'
    exit
fi
# -----------------------------------------------------------------------------
if [ "$branch2" != '' ] || [ -d new ]
...
do
    ...
    # ----------------------------------------------------------------------
    # run cmake
    echo "tools/run_cmake.sh > build.release/$name.log"
    tools/run_cmake.sh > build/$name.log
    #
    if [ "$name" != 'master' ]
    then
        # ------------------------------------------------------------------
        ...
        then
            echo "tools/get_cppad_mixed.sh >> build.release/$name.log"
            tools/get_cppad_mixed.sh >> build/$name.log
        fi
    fi
    # ------------------------------------------------------------------------
    # build dismod_at
