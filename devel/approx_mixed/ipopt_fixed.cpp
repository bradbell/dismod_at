	...
	Number*         lambda       )  // out
{
	...
	{	fix_like_vec_tmp_ = approx_object_.prior_eval(fixed_in_);
...
{	bool ok = true;
	...
	{	fix_like_vec_tmp_ = approx_object_.prior_eval(fixed_opt_);
		assert( fix_like_vec_tmp_.size() == 1 + fix_like_n_abs_ );
